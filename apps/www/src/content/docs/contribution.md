---
title: Contribution
description: Learn on how to contribute to shadvue.
---
...
2. **apps/www** -
The main folder that holds the source code for the website and every `shadvue` component. This folder contains important sub-folders and is a subproject with its own `package.json`.

3. **.vitepress** -
Contains the configuration and source code for `vitepress` and the `shadvue` website.

4. **src** -
Hosts the main source code for every `shadvue` component or demo and their documentation on the website.
...
We support two different styles for every component in `shadvue`:
...
### CSS Classes
There are cases when we want to accept `class` as a prop in our `shadvue` component and then combine it with a default tailwind class on our `Reka UI` component via `cn` utility function.
...
## Updating with `shadvue`

`shadvue` is an unofficial, community-led Vue port of `shadcn/ui`, as time goes by, they might get out of sync.

As of today, we are in sync with this <a :href="latestSyncCommitUrl" target="_blank">commit</a> of `shadvue`.
This commit is tagged as `v0.0.1`.
...
2. If there are changes, you should review thoese changes and try to apply them on `shadvue` codebase and create a PR, remember to update the `latestSyncCommitTag` in [this file](https://github.com/unovue/shadcn-vue/blob/dev/apps/www/src/content/docs/contribution.md) too.

## Debugging
Here are some tools and techniques that can help you debug more effectively while contributing to `shadvue` or developing your own projects.
